...
def get_all_scores(query_vectors: np.array, ctx_embeddings):

    cls_vectors = np.zeros((len(ctx_embeddings), 768))
    pids = []
    for i, ctx_embedding in enumerate(ctx_embeddings):
        pid, _cls = ctx_embedding
        cls_vectors[i] = _cls
        pids.append(pid)
    pids = np.array(pids)

    # with open('ctx_embedding_output/pids', 'wb') as handle:
    ...
    for query_vector in query_vectors:
        ...
        cls_scores = np.dot(query_vector, cls_vectors.T)
        results.append(cls_scores)
    
    # with open('ctx_embedding_output/student_scores', 'wb') as handle:
...
def main(args):
    ...
    # with open(args.encoded_ctx_file, 'rb') as handle:
    #     ctx_embeddings = pickle.load(handle)
    # get_all_scores(questions_tensor, ctx_embeddings)

    # get top k results
    top_ids_and_scores = retriever.get_top_docs(questions_tensor.numpy(), args.n_docs)

    with open(args.out_file, 'wb') as handle:
        pickle.dump(top_ids_and_scores, handle, protocol=pickle.HIGHEST_PROTOCOL)
    print('saving for msmarco')
    exit(1)
